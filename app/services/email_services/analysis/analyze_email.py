...
from app.utils.remediation_utils import get_remediation_suggestion_for_severity
from app.utils.get_severity_for_risk_factor import get_severity_for_risk_factor 
...
def save_analysis_results(email, phishing_result, db_session: Session):
    ...
    try:
        ...
        threat_analysis = ThreatAnalysis(
            email_id=email.id,
            is_threat=(phishing_result.get('classification') != ThreatTypeEnum.SAFE),
            threat_type=phishing_result.get('classification'),
            severity=phishing_result.get('severity'),
        ...
        # Assume phishing_result['risk_factors'] is a list of strings generated by the phishing detector.
        if phishing_result.get('risk_factors'):
            for factor in phishing_result['risk_factors']:
                # For now, we assign a standard highlight type.
                highlight_type = "Phishing Indicator"
                content = factor
                # Dynamically determine severity based on the factor content.
                factor_severity = get_severity_for_risk_factor(factor)
                description = f"Indicator detected: {factor}"
                remediation_suggestion = get_remediation_suggestion_for_severity(factor_severity)
